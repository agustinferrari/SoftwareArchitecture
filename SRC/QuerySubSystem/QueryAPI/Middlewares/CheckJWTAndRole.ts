  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (roles.indexOf(user.role) > -1) {
        logger.logAuthorizedAccess("User authorized to access.  Authorized roles: " + roles.join(", "), req.originalUrl, user);
        res.locals.userDTO = user;
        next();
      } else {
        logger.logUnauthorizedAccess("User not authorized.  Authorized roles: " + roles.join(", "), req.originalUrl, user);
