...
export class RepeatedVoteFElectionVoteDatesFilterilter implements IFilter {
  electionId: any;
  key: any;
  error: string;
  maxAttempts: number;
  ...
  async validate() {
    let isValid = await this.electionQuery.validateElectionVotesDate(this.electionId);
    if (!isValid) {
